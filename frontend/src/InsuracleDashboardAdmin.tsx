...
export default function InsuracleDashboardAdmin({ setUserType }: ParamifyDashboardProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { sellerAddress, contractUsdcPool: pool, contractUsycReserve: reserve } = statusRes.data;
      ...
      if (sellerAddress) {
        const balRes = await axios.get(`${API_BASE}/api/balance/${sellerAddress}`);
        setAdminUsdcBalance(balRes.data.usdc || '0');
        setAdminUsycBalance(balRes.data.usyc || '0');
      }
    } catch (e) {
      console.error('Failed to fetch admin data:', e);
      setTransactionStatus('Failed to load admin data - is the backend running?');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDepositReserve = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/api/admin/deposit-reserve`, { amountUsyc: amount });
      if (res.data.success) {
        setTransactionStatus(`Deposited ${amount} USYC to reserve. Tx: ${res.data.txId}`);
        setReserveAmount('');
  ...
  const handleTriggerClaim = async () => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/api/admin/trigger-claim`, {
        recipientAddress: claimRecipient,
        amountUsdc: parseFloat(claimAmount),
      });
      if (res.data.success) {
        setTransactionStatus(`Claim triggered! ${claimAmount} USDC sent to ${claimRecipient.slice(0, 10)}... Tx: ${res.data.txId}`);
        setClaimRecipient('');
        setClaimAmount('');
        setTimeout(() => fetchData(), 4000);
      }
