...
// Paramify ABI (pool/reserve reads for admin dashboard)
const PARAMIFY_ABI = [
  {
    "inputs": [],
    "name": "usdcPool",
  ...
  try {
    ...
    try {
      ...
      const [usdcPool, usycReserve] = await Promise.all([
        paramifyContract.usdcPool(),
        paramifyContract.usycReserve(),
      ]);
      poolData = {
...
app.post('/api/admin/deposit-reserve', async (req, res) => {
  try {
    ...
    // Step 1: Approve Paramify to spend USYC from Circle dev wallet
    await circleClient.createContractExecutionTransaction({
      walletId: process.env.CIRCLE_WALLET_ID,
      contractAddress: USYC_ADDRESS,
      abiFunctionSignature: 'approve(address,uint256)',
      abiParameters: [PARAMIFY_ADDRESS, MAX_UINT256],
    ...
    // Step 2: Call depositReserve on Paramify
    const depositTx = await circleClient.createContractExecutionTransaction({
      walletId: process.env.CIRCLE_WALLET_ID,
      contractAddress: PARAMIFY_ADDRESS,
